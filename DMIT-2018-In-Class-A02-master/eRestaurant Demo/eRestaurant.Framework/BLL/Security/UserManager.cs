...
namespace eRestaurant.Framework.BLL.Security
{
    ...
    {
        #region Constants
    // Ideally, we should put some/all of these in the Web.config file underneath the <appSettings> element.
            ...
            public void AddDefaultUsers()
            {
                using (var context = new RestaurantContext())
                {
                    ...
                    foreach (var person in employees)
                    {
                        ...
                        // if (NOT any of the users exist such that(=>) u(represents a human user) where the
                        // user's WaiterID has a value AND the user's WaiterID matches this person's WaiterID)
                        if (!Users.Any(u => u.WaiterID.HasValue && u.WaiterID.Value == person.WaiterID))
                        {
                            ...
                            // NOTE: The following line of code needs to use the this keyword in order to have
                            // access to the Create() extension method.
